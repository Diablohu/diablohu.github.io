 ...
 */
$(function() {
    /*var dialog = Global.dialog(true,'头像修改成功');
     $('body').append(dialog);*/
    var sid = getCookie('session_aone');
    var uid = $("#uid").val();

    function setHead(imageUrl) {
        $.ajax({
            url:'',
            type:'POST',
            data: {
    ...
    /*$('.cancel-btn').on('click', function() {
     $('.cropper-wraper').fadeOut('fast', function () {});
     });*/
        ...
        return {
            init: function( selectCb ) {
                uploader = new Uploader({
                    ...
                    server: 'http://login.prpracg.com/api/member/edit_head?sid='+sid+'&uid='+uid,
                    swf: 'http://img.acgn.prpracg.com/tms/jcrop/Uploader.swf',
                    fileNumLimit: 1,
                    onError: function() {
        ...
        return {
            init: function( selectCb ) {
                ...
                uploader.on('beforeFileQueued', function (file) {
                    var hasFile = uploader.getFiles();
                    if (hasFile.length === 1) {
                        uploader.removeFile(hasFile[0]);
                        uploader.reset();
                    }
                });
                ...
                uploader.on('uploadFinished', function () {
                    // 重新设置状态
                    uploadFile.setStatus('queued');
                });

                uploader.on('uploadSuccess', function(_file, data) {
                    if(data.code == 1)
                    {
...
// ---------------------------------
    var Croper = (function() {
        ...
        btn.on('click', function() {
            if (jQuery('img', '.img-preview').length) {
                callback && callback($image.cropper('getData'));
                // console.log('执行这个了');
            }
            else {
                var imageUrl = $('.img-preview').attr('defaultImgUrl');
                setHead(imageUrl);
            }
            return false;
        });
