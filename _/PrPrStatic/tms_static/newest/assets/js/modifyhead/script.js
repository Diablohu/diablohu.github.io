 ...
 */
$(function() {
    var dialog = Global.dialog.getHtml({'iscorrect':true,'title':"成功"});
     $('body').append(dialog);
            ...
            init: function( selectCb ) {
                ...
                uploader.on('fileQueued', function( _file ) {
                    ...
                    uploader.makeThumb( file, function( error, src ) {
                        ...
                        if ( error ) {
                            $('.cropper-modal').text('不能预览');

                        } else {
                            $('.cropper-modal').append('&lt;img alt="" src="' + src + '" />');
                        }
            ...
            init: function( selectCb ) {
                ...
                uploader.on('uploadAccept', function (file, response) {
                    ...
                    if (response.status === 0) {
                        var imageUrl = response.data.imageInfo.url;
                        //$('.img-preview').attr('defaultImgUrl', imageUrl);
                        setProfile(imageUrl);
                    }
                });
            },
